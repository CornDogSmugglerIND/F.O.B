...
describe("identify router", () => {
  ...
  it("photo path is honest when vision key is missing — no key ask, no barcode fallback", async () => {
  ...
  it("does not treat a bare barcode as Identify", async () => {
    const result = await runIdentify({ barcode: "045496590425" });
    assert.equal(result.ok, false);
    assert.equal(result.path, "none");
    assert.match(result.message, /separate/i);
  });

  it("barcode only runs when forcePath is barcode", async () => {
    const result = await runIdentify({ barcode: "000", forcePath: "barcode" });
